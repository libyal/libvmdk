...
LIBRARY_TESTS="descriptor_file error extent_descriptor extent_file extent_table grain_data grain_group grain_table io_handle notify";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./vmdk_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libvmdk" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
