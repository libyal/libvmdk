...
then
	docker run -t -v "${PWD}:/libvmdk" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libvmdk && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libvmdk;
