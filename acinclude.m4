...
AC_DEFUN([LIBVMDK_TEST_ENABLE],
	[AC_ARG_ENABLE(
	 [$1],
	 [AS_HELP_STRING(
	  [--enable-$1],
	  [$3 (default is $4)])],
	 [ac_cv_libvmdk_enable_$2=$enableval],
	 [ac_cv_libvmdk_enable_$2=$4])dnl
	 AC_CACHE_CHECK(
	  [whether to enable $3],
	  [ac_cv_libvmdk_enable_$2],
	  [ac_cv_libvmdk_enable_$2=$4])dnl
])

dnl Function to detect whether nl_langinfo supports CODESET
AC_DEFUN([LIBVMDK_LANGINFO_CODESET],
	[AC_CHECK_FUNCS(
	 [nl_langinfo],
	 [AC_CACHE_CHECK(
	  [for nl_langinfo CODESET support],
	  [ac_cv_libvmdk_langinfo_codeset],
	  [AC_LANG_PUSH(C)
	  AC_LINK_IFELSE(
	   [AC_LANG_PROGRAM(
	    [[#include <langinfo.h>]],
	    [[char* charset = nl_langinfo( CODESET );]] )],
	   [ac_cv_libvmdk_langinfo_codeset=yes],
	   [ac_cv_libvmdk_langinfo_codeset=no])
	  AC_LANG_POP(C) ]) ],
	 [ac_cv_libvmdk_langinfo_codeset=no] )
	AS_IF(
	 [test "x$ac_cv_libvmdk_langinfo_codeset" = xyes],
	 [AC_DEFINE(
...
AC_DEFUN([LIBVMDK_CHECK_FUNC_CTIME],
	[AC_CHECK_FUNC(
	 [ctime_r],
	 [AC_LANG_PUSH(C)
	 AC_MSG_CHECKING(
