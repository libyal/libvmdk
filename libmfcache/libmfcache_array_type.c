...
{
	static char *function = "libmfcache_array_initialize";
	...
	*array = memory_allocate_structure(
	...
	if( memory_set(
	     *array,
	     0,
	     sizeof( libmfcache_array_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int libmfcache_array_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_free";
	int result            = 1;
...
int libmfcache_array_empty(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_empty";
...
int libmfcache_array_clear(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_clear";
...
int libmfcache_array_clone(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_clone";
	int entry_iterator    = 0;
     ...
     libcerror_error_t **error )
{
	...
	static char *function = "libmfcache_array_resize";
	size_t entries_size   = 0;
	int entry_iterator    = 0;
	int result            = 1;
...
int libmfcache_array_get_number_of_entries(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_get_number_of_entries";
...
int libmfcache_array_get_entry_by_index(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_get_entry_by_index";
...
int libmfcache_array_set_entry_by_index(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_set_entry_by_index";
...
int libmfcache_array_append_entry(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_array_append_entry";
 ...
 * The entry_compare_function should return LIBMFCACHE_ARRAY_COMPARE_LESS,
 * LIBMFCACHE_ARRAY_COMPARE_EQUAL, LIBMFCACHE_ARRAY_COMPARE_GREATER if successful or -1 on error
 *
 * Duplicate entries are allowed by default and inserted after the last duplicate entry.
 * Only allowing unique entries can be enforced by setting the flag LIBMFCACHE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES
 *
...
int libmfcache_array_insert_entry(
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfcache_tree_node_insert_node";
	int entry_iterator    = 0;
	int result            = -1;
	...
	if( ( insert_flags & ~( LIBMFCACHE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) ) != 0 )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( array->entries != NULL )
	{
		     ...
		     entry_iterator++ )
		{
			...
			else if( result == LIBMFCACHE_ARRAY_COMPARE_EQUAL )
			{
				if( ( insert_flags & LIBMFCACHE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) != 0 )
				{
					return( 0 );
			...
			else if( result == LIBMFCACHE_ARRAY_COMPARE_LESS )
			{
				break;
			}
			else if( result != LIBMFCACHE_ARRAY_COMPARE_GREATER )
			{
				libcerror_error_set(
				 error,
				 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( ( array->entries != NULL )
	 && ( result == LIBMFCACHE_ARRAY_COMPARE_LESS ) )
	{
