 ...
 * Returns 1 if successful or -1 on error
 */
int libmfdata_array_clone(
     libmfdata_array_t **destination_array,
     libmfdata_array_t *source_array,
     int (*entry_free_function)(
            intptr_t **entry,
            libcerror_error_t **error ),
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfdata_array_clone";
	int entry_iterator    = 0;
 ...
 * The entry_compare_function should return LIBMFDATA_ARRAY_COMPARE_LESS,
 * LIBMFDATA_ARRAY_COMPARE_EQUAL, LIBMFDATA_ARRAY_COMPARE_GREATER if successful or -1 on error
 *
 * Duplicate entries are allowed by default and inserted after the last duplicate entry.
 * Only allowing unique entries can be enforced by setting the flag LIBMFDATA_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES
 *
 * Returns 1 if successful, 0 if the node already exists or -1 on error
 */
int libmfdata_array_insert_entry(
     libmfdata_array_t *array,
     int *entry_index,
     intptr_t *entry,
     ...
     libcerror_error_t **error )
{
	static char *function = "libmfdata_tree_node_insert_node";
	int entry_iterator    = 0;
	int result            = -1;
	...
	if( ( insert_flags & ~( LIBMFDATA_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) ) != 0 )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( array->entries != NULL )
	{
		     ...
		     entry_iterator++ )
		{
			...
			else if( result == LIBMFDATA_ARRAY_COMPARE_EQUAL )
			{
				if( ( insert_flags & LIBMFDATA_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) != 0 )
				{
					return( 0 );
			...
			else if( result == LIBMFDATA_ARRAY_COMPARE_LESS )
			{
				break;
			}
			else if( result != LIBMFDATA_ARRAY_COMPARE_GREATER )
			{
				libcerror_error_set(
				 error,
				 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( ( array->entries != NULL )
	 && ( result == LIBMFDATA_ARRAY_COMPARE_LESS ) )
	{
