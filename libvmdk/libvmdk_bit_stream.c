...
{
	static char *function = "libvmdk_bit_stream_initialize";
	...
	if( ( storage_type != LIBVMDK_BIT_STREAM_STORAGE_TYPE_BYTE_BACK_TO_FRONT )
	 && ( storage_type != LIBVMDK_BIT_STREAM_STORAGE_TYPE_BYTE_FRONT_TO_BACK ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	*bit_stream = memory_allocate_structure(
	...
	if( memory_set(
	     *bit_stream,
	     0,
	     sizeof( libvmdk_bit_stream_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int libvmdk_bit_stream_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libvmdk_bit_stream_free";
...
int libvmdk_bit_stream_get_value(
     ...
     libcerror_error_t **error )
{
	static char *function            = "libvmdk_bit_stream_get_value";
	...
	while( remaining_number_of_bits > 0 )
	{
		    ...
		    && ( bit_stream->bit_buffer_size <= 24 ) )
		{
			...
			else if( bit_stream->storage_type == LIBVMDK_BIT_STREAM_STORAGE_TYPE_BYTE_FRONT_TO_BACK )
			{
