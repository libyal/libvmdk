      ...
      pyvmdk_extent_descriptors_t *pyvmdk_extent_descriptors )
{
	...
	if( pyvmdk_extent_descriptors->ob_type == NULL )
	{
		PyErr_Format(
		 PyExc_ValueError,
		 "%s: invalid extent descriptors - missing ob_type.",
		 function );

		return;
	}
	if( pyvmdk_extent_descriptors->ob_type->tp_free == NULL )
	{
		PyErr_Format(
		 PyExc_ValueError,
		 "%s: invalid extent descriptors - invalid ob_type - missing tp_free.",
		 function );
	...
	pyvmdk_extent_descriptors->ob_type->tp_free(
	 (PyObject*) pyvmdk_extent_descriptors );
}
